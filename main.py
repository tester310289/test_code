...
from get_args import args_gets
from gat import GAT
...
def data_process(args):
    ...
    print(type(features), np.shape(features))
    ...
    return g, num_feats, cuda, features, adj, labels, n_classes


def train(args, g, num_feats, cuda, features, adj):
    # create model
...
def gcl_test(args, model, features, labels, n_classes):
    model.eval()
    with torch.no_grad():
        heads = model(features)
...
def main():
    args = args_gets()
    g, num_feats, cuda, features, adj, labels, n_classes = data_process(args)
    model = train(args, g, num_feats, cuda, features, adj)
    gcl_test(args, model, features, labels, n_classes)
